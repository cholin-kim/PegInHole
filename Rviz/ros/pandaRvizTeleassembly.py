...
class pandaRvizTeleassembly:
    def __init__(self, is_core=True, use_GUI=False, base_transform=None):
        ...
        if base_transform is None:
            ...
            self.xyz, self.rpy, self.angle, self.radius = base_transform

        uuid = roslaunch.rlutil.get_or_generate_uuid(None, False)
        roslaunch.configure_logging(uuid)

        args = ['xyz:='+self.xyz, 'rpy:='+self.rpy,
                'angle:='+self.angle, 'radius:='+self.radius,
                'path:='+path]

        if use_GUI:
            launch = roslaunch.parent.ROSLaunchParent(uuid, [(path + "/model_description/launch/visualize_panda_dual_GUI.launch", args)], is_core=is_core)
        else:
            launch = roslaunch.parent.ROSLaunchParent(uuid, [(path + "/model_description/launch/visualize_panda_dual.launch", args)], is_core=is_core)

        launch.start()
        time.sleep(1)

        if not rospy.get_node_uri():
            rospy.init_node("panda_visualizer", anonymous=True)
        else:
...
if __name__ == "__main__":
    ...
    from panda_teleassembly.ros.pandaRvizMarkerTeleassembly import pandaRvizMarkerTeleassembly
    ...
    while True:
        ...
        arm_des2[1] = 0.0# + 0.7 * np.cos(0.2 * np.pi * t)
        ...
        env_sim.set_joint_position_direct(joints=[0.0 + 1.0 * np.cos(0.7 * np.pi * t)])
        marking.set_points(id=id, L_marker=L_point)
